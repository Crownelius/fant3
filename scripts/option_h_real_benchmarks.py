"""
Option H — Real public-benchmark eval (GSM8K + MMLU + ARC-Easy + ARC-Challenge).

This script EVALUATES (does NOT train on) public benchmarks. Per the
no-public-benchmarks-for-training constraint, eval is allowed; training is
not. The result tells us where the current ramped tiny checkpoint sits on
each task.

Honest baseline expectations for `output/option_g/ramp/final.pt`:
  * 5M-param tiny preset (dim=128, 4 layers, max_seq_len=128)
  * Trained almost exclusively on `SyntheticStream` (a 20-sentence seed
    corpus + procedural math), tokenizer is a 856-token domain BPE
  * Real natural-language English fragments into ~3-4 tokens per word at
    this vocab, vs ~1.3 for a real BPE → most tokens the model sees on
    these benchmarks are byte-level fragments it has never been trained
    on in the relevant context
  * GSM8K (greedy generation): ~0% — model can't produce coherent CoT
  * MMLU / ARC (4-choice scoring): ~25% (random) is the floor; anything
    materially above 25% would be a mild signal that the bench harness is
    discriminating SOMETHING (length norms, common-token frequencies)
  * The point of this script is to validate the bench-harness wiring on
    real benchmarks. Real numbers come after a real Phase 1+2 pretraining
    run on FineWeb-Edu (out of scope for the zesty-discovering-key plan)

Run:
    PYTHONPATH=. python scripts/option_h_real_benchmarks.py
"""
...
CKPT_PATH = "output/option_g/ramp/final.pt"
TOK_PATH  = "output/option_e/tokenizer.json"
OUT_DIR   = "output/option_h"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--gsm8k-n", type=int, default=50,
                   help="Number of GSM8K problems (default 50)")
...
def run_mmlu(model, tokenizer, n: int, device: str) -> dict:
    ...
    print(f"    MMLU: {res['correct']}/{res['total']} = {res['accuracy']:.1%} "
          f"({res['wall_seconds']:.0f}s)")
    # Drop per_subject from the printed summary; keep it in the JSON
    return res
...
def main() -> int:
    ...
    print("=" * 64)
    print(" FANT 2 — Option H: real public-benchmark eval")
    print(" (eval-only — no training touches these benchmarks)")
    print("=" * 64)

    if not os.path.exists(CKPT_PATH):
        print(f"  ✗ checkpoint not found at {CKPT_PATH}")
        print(f"    run scripts/option_g_benchmark_rampup.py first")
        return 1
    ...
    # ---- run each benchmark, catching individual failures so the others
    # ---- still complete. We always want a full row even if one breaks.
    for name, fn in [
    ...
    # 4-choice tasks have a 25% random baseline. Flag if anything is far above.
    print("  reference baselines:")
    print("    GSM8K random         ≈ 0.0% (open-ended, no number = wrong)")
    print("    MMLU random          ≈ 25%  (4 choices, length-normed scoring)")
    print("    ARC-Easy random      ≈ 25%")
    print("    ARC-Challenge random ≈ 25%")
    print()
