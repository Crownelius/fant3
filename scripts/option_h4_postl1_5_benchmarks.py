"""
Option H4 — Re-bench against the Option L1.5 checkpoint.

Same harness as H3, only the checkpoint path changes. Option L1.5 =
Option I -> Phase 4 procedural-math ramp WITH curvature_threshold=1.0
(fixes the ApollonianBetaStarvation that left beta=0 in L1). L1.5 hit
61.5% on procedural math (vs L1's 54.5% and K's 80.0%), and has
alpha_fill=beta_fill=1.0 for the first time in the codebase's history.

  CKPT  : output/option_l1_5/math_ramp/final.pt (Option I -> Option L1.5)
  TOK   : output/option_i/tokenizer.json         (32K BPE on FineWeb-Edu)

Comparison target — Option H3 (Option L1 checkpoint):
    GSM8K          2.0%
    MMLU          24.0%
    ARC-Easy      20.0%
    ARC-Challenge 27.5%
...
Run:
    PYTHONPATH=. python scripts/option_h4_postl1_5_benchmarks.py
"""
...
# -----------------------------------------------------------------------------
# Configuration — Option I tokenizer + Option L1.5 checkpoint
# -----------------------------------------------------------------------------

CKPT_PATH = "output/option_l1_5/math_ramp/final.pt"
TOK_PATH  = "output/option_i/tokenizer.json"
OUT_DIR   = "output/option_h4"
...
def main() -> int:
    ...
    print("=" * 64)
    print(" FANT 2 — Option H4: re-bench against Option L1.5 checkpoint")
    print(" (eval-only — no training touches these benchmarks)")
    print("=" * 64)

    if not os.path.exists(CKPT_PATH):
        print(f"  ✗ checkpoint not found at {CKPT_PATH}")
        print(f"    run scripts/option_l1_5_curvature_fix.py first")
        return 1
    ...
    print()
    print("=" * 64)
    print(" RESULTS — Option H4 (Option L1.5 checkpoint)")
    print("=" * 64)
    ...
    print("  Option H3 baseline (Option L1 = phase=4, beta=0):")
    print("    GSM8K          2.0%")
    print("    MMLU          24.0%")
    print("    ARC-Easy      20.0%")
    print("    ARC-Challenge 27.5%")
    print()
    print("  Option H2 baseline (Option K = phase=2, memory dormant):")
    print("    GSM8K          1.0%")
    print("    MMLU          24.5%")
    print("    ARC-Easy      24.5%")
