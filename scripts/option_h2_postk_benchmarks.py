"""
Option H2 — Re-bench against the Option I+K checkpoint.

Same harness as `option_h_real_benchmarks.py` (GSM8K + MMLU + ARC-Easy +
ARC-Challenge via streaming HF datasets, length-normed for the multi-choice
tasks). Only the checkpoint and tokenizer paths change:

  CKPT  : output/option_k/math_ramp/final.pt   (Option I -> Option K)
  TOK   : output/option_i/tokenizer.json       (32K BPE on FineWeb-Edu)

Why a fresh script instead of CLI args on Option H: keeps the Option H
baseline run preserved for comparison, and makes the JSON output land in
its own directory (`output/option_h2/results.json`) so the two runs are
side-by-side rather than overwriting.

The expected delta vs Option H:
  * GSM8K: same harness, much better tokenizer, real-text pretraining.
    Should rise off 1% if the procedural-math ramp generalizes at all.
  * MMLU: still ~25% — 5M params is too small for 57-subject knowledge.
  * ARC-Easy / ARC-Challenge: limited by the un-normed scorer in
    `bench/arc.py` (see Option H postmortem). Will only meaningfully move
    if the model has acquired enough text signal that the length bias
    stops dominating.

Run:
    PYTHONPATH=. python scripts/option_h2_postk_benchmarks.py
"""
...
# -----------------------------------------------------------------------------
# Configuration — Option I tokenizer + Option K checkpoint
# -----------------------------------------------------------------------------

CKPT_PATH = "output/option_k/math_ramp/final.pt"
TOK_PATH  = "output/option_i/tokenizer.json"
OUT_DIR   = "output/option_h2"
...
def main() -> int:
    ...
    print("=" * 64)
    print(" FANT 2 — Option H2: re-bench against Option I+K checkpoint")
    print(" (eval-only — no training touches these benchmarks)")
    print("=" * 64)

    if not os.path.exists(CKPT_PATH):
        print(f"  ✗ checkpoint not found at {CKPT_PATH}")
        print(f"    run scripts/option_k_procedural_ramp.py first")
        return 1
    ...
    print()
    print("=" * 64)
    print(" RESULTS — Option H2 (Option I+K checkpoint)")
    print("=" * 64)
    ...
    print("  Option H baseline (output/option_g/ramp/final.pt, 856-vocab BPE):")
    print("    GSM8K          1.0%")
    print("    MMLU          25.0%")
    print("    ARC-Easy      26.5%")
    print("    ARC-Challenge 18.0%")
    print()
