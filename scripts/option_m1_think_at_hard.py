"""
Option M1 — Phase 4 ramp + Think-at-Hard per-token pass-2 gate + Phi-4 curriculum.

Context: L1.5 fixed the β starvation bug (α fill 100% → β fill 100%) but Phase 4
still underperforms Phase 2 by -18.5pp on procedural math. The literature scan
(research/literature_scan_2026-04-09.md) identified Think-at-Hard (arxiv:2511.08577)
as the single highest-signal paper for this regression: it explicitly names the
failure mode ("latent overthinking") and provides a concrete per-token gate fix.

M1 adds two features on top of the L1.5 recipe:

  #1 Think-at-Hard per-token pass-2 gate (arxiv:2511.08577)
     Pass-2 CE loss is masked on tokens where pass-1 softmax confidence exceeds
     phase4_gate_threshold. This implements the Think-at-Hard observation that
     ~94% of tokens are already correct after pass 1 and forcing pass 2 on them
     corrupts the correct outputs. We start with a threshold of 0.7 — tokens with
     max-softmax > 0.7 skip the pass-2 gradient entirely.

  #5 Phi-4-style difficulty filter (arxiv:2504.21233)
     Rejection-filter procedural math problems so only the middle difficulty band
     (5 ≤ gold_answer ≤ 80) contributes to training. Excludes trivially-easy
     problems (gold < 5) that give ~zero gradient and excludes problems with
     operand magnitude the 5M tiny model cannot solve (gold > 80).

Everything else matches L1.5:
  - Start from Option I checkpoint (output/option_i/pretrain/final.pt)
  - fant2_tiny preset, phase=4, 2500 steps, seed 11
  - curvature_threshold = 1.0 (L1.5 β starvation fix)
  - output_gate_init = 0.1

Baselines to beat:
    K (phase 2 only):                  80.0% procedural
    L1  (phase 4, β=0 bug):            54.5% procedural
    L1.5 (phase 4, β fixed):           61.5% procedural  ← M1 target

Run:
    PYTHONPATH=. python scripts/option_m1_think_at_hard.py
"""
...
OUTPUT_GATE_INIT     = 0.1
CURVATURE_THRESHOLD  = 1.0  # L1.5 β starvation fix, kept for M1

# Option M feature flags
PHASE4_GATE_ENABLED   = True
PHASE4_GATE_THRESHOLD = 0.7   # Think-at-Hard: skip pass-2 gradient for tokens with pass-1 conf > 0.7
PHASE4_PREPEND_K      = 0     # legacy pooled-mean feedback; M2 turns this on
PHASE4_ALIGNMENT_WT   = 0.0   # legacy MSE consistency only; M2 turns this on
PHASE4_CLASS_UPSTREAM = False # legacy post-RMSNorm classifier; M3 turns this on
PHASE4_CLASS_MODE     = "curvature"  # legacy L2-norm proxy; M3 turns this to ce_surprise

# Phi-4 difficulty filter bounds for #5
...
class DifficultyFilteredMathStream:
    """
    Wrap a ProceduralMathStream and yield only problems whose integer gold
    answer falls in the [min_ans, max_ans] band. This is the Phi-4-Reasoning
    pattern (arxiv:2504.21233) of filtering training examples by difficulty
    so the model gets gradient on problems that are neither trivially easy
    nor impossibly hard.
    """
...
# -----------------------------------------------------------------------------
# Procedural math eval — same as L1.5 (no difficulty filter; measures the
# full natural distribution so M1 numbers are comparable to K / L1 / L1.5)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Memory subsystem instrumentation (same as L1.5)
# -----------------------------------------------------------------------------
...
def apply_m_flags(cfg) -> None:
    """Apply the Option M1 phase-4 feature flags to a FANT2Config."""
...
def main() -> int:
    print("=" * 64)
    print(" FANT 2 — Option M1: Think-at-Hard gate + Phi-4 difficulty filter")
    print(" Phase 4 ramp on L1.5 recipe + #1 #5 from literature scan")
    print("=" * 64)
    print()
    print(f"  phase4_gate_enabled       = {PHASE4_GATE_ENABLED}")
    print(f"  phase4_gate_threshold     = {PHASE4_GATE_THRESHOLD}")
    print(f"  phase4_prepend_k          = {PHASE4_PREPEND_K} (legacy pooled; M2 turns this on)")
    print(f"  phase4_alignment_weight   = {PHASE4_ALIGNMENT_WT} (legacy MSE; M2 turns this on)")
    ...
    print()
    print("=" * 64)
    print(" RESULTS — Option M1 (Think-at-Hard + difficulty filter)")
    print("=" * 64)
    ...
    print(f"    M1  (phase 4+gate): {post_res['correct']}/{post_res['total']} = "
